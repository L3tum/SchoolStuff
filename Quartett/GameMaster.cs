...
namespace Quartett
{
	public class GameMaster
	{
		...
		public Karte[] GetGespielteKarten()
		{
			return gespielteKarten.Values.ToArray();
		}
		...
		public void Run()
		{
			...
			while (spieler.Length > 1)
			{
				...
				if (!monitored)
				{
					var info = new List<Tuple<string, string, string>>();

					foreach (KeyValuePair<int, Karte> pair in gespielteKarten)
					{
						info.Add(new Tuple<string, string, string>("Spieler" + pair.Key,
							pair.Value.GetProperty("identifier").ToString(),
							pair.Value.GetProperty(aktuelleEigenschaft).ToString()));
					}

					Console.WriteLine(info.ToStringTable(new[] {"Spieler", "Karte", aktuelleEigenschaft},
						tuple => tuple.Item1,
						tuple => tuple.Item2,
						tuple => tuple.Item3));
				}
		...
		private void SetNumberOfPlayers()
		{
			...
			if (kiNumber > 0)
			{
				Console.Write("Soll die schwere KI geladen werden? [y/N] ");
			...
			if (kiNumber > 0 && number == 0)
			{
				Console.Write("Soll das Programm eigenständig ablaufen? [y/N] ");
				var yn = Console.ReadLine();
